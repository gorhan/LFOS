...
class Schedule:
    ...
    def plot_schedule(self):
        ...
        for ind, (resource, reservations) in enumerate(sorted(self.__schedule.items(), key=lambda r: r[0].get_resource_name(), reverse=True)):
        ...
        ax.set_yticklabels(['%s%s' % (resource.get_resource_name(), '*' if resource.get_type().get_abstraction() == ResourceTypeList.ACTIVE else '') for resource in sorted(self.__schedule.keys(), key=lambda r: r.get_resource_name(), reverse=True)])
