from LFOS.Resource.Resource import ACTIVE,ResourceInterface
from LFOS.Resource import Request
...
class AbstractTask(Timing, Request, Priority):
    ...
    def add_eligible_resource(self, resource, wcet_max_scale):
        if not isinstance(resource, ResourceInterface):
            LOG(msg='Given parameter is not a resource.', log=Logs.ERROR)
            return False

        if resource.type.same_abstraction(ACTIVE) and resource not in self.eligible_resources:
            ...
            return True
        elif resource.type.same_abstraction(ACTIVE):
